...
import { getSubscriptions, updateSubscription } from "@/lib/services/subscription"
...
export default function MyPage() {
  ...
  const { emailSubscribe: emailNotifications, setEmailSubscribe: setEmailNotifications } = useEmailSubscribe(true)
  const [tours, setTours] = useState<Tour[]>([])
  const [visibleEvents, setVisibleEvents] = useState(6)
  const [isSubscriptionModalOpen, setIsSubscriptionModalOpen] = useState(false)
  const [subscribedRegions, setSubscribedRegions] = useState<Record<string, string[]>>({})
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
              ...
              <CardContent className="p-6">
                <div className="mb-6">
                  <div className="text-center mb-4">
                    ...
                    <h3 className="text-lg font-semibold text-foreground mb-2">메일 알림 설정</h3>
                    <p className="text-sm text-muted-foreground mb-3">구독한 지역의 축제 정보를 이메일로 받아보세요</p>
                  </div>

                  {/* 이메일 주소 표시 */}
                  <div className="bg-muted/50 rounded-lg p-3 mb-4">
                    <div className="text-center">
                      <p className="text-sm text-muted-foreground mb-1">알림 받을 이메일</p>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between p-3 bg-muted/30 rounded-lg">
                    <div>
                      <span className="text-sm font-medium">이메일로 알림받기</span>
                      <p className="text-xs text-muted-foreground">새로운 축제 정보를 메일로 받기</p>
                    </div>
                    ...
                    <AlertDialog>
                      ...
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          ...
                          <AlertDialogDescription>
                            탈퇴 시 모든 구독 및 계정 정보가 삭제됩니다. 이 작업은 되돌릴 수 없습니다.
                          </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
          ...
          <div className="lg:col-span-3">
            <Card>
              <CardContent className="p-6">
                <div className="mb-6">
                  <h1 className="text-3xl font-bold text-foreground mb-2">구독 지역의 축제</h1>
                  <p className="text-muted-foreground">구독하신 지역에서 열리는 다양한 행사들을 확인해보세요</p>
                </div>
                ...
                {isLoadingEvents ? (
                  <div className="text-center py-8">
                    <div className="inline-flex items-center space-x-2 text-muted-foreground">
                      ...
                      ></div>
                      <span className="ml-2 text-sm">축제 정보를 불러오는 중...</span>
                    </div>
                  </div>
                ) : tours.length === 0 ? (
                  <div className="text-center py-8">
                    <p className="text-muted-foreground">구독한 지역의 축제가 없습니다.</p>
                    <p className="text-sm text-muted-foreground mt-2">구독 지역을 추가해보세요!</p>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {tours.slice(0, visibleEvents).map((tour) => (
                    <Card key={tour.tourId} className="overflow-hidden hover:shadow-lg transition-shadow group">
                      {/* 이미지 영역 */}
                      ...
                      <div className="p-4 space-y-2">
                        {/* 제목 */}
                        <h3 className="text-lg font-semibold text-foreground line-clamp-1 -mt-1">{tour.title}</h3>

                        {/* 날짜 */}
                        <div className="flex items-center text-sm text-muted-foreground">
                          <Calendar className="w-4 h-4 mr-2" />
                          <span>{tour.startDate === tour.endDate ? tour.startDate : `${tour.startDate} - ${tour.endDate}`}</span>
                        </div>

                        {/* 위치 */}
                        <div className="flex items-center text-sm text-muted-foreground">
                          <MapPin className="w-4 h-4 mr-2" />
                          <span>{tour.area} {tour.sigunGu}</span>
                        </div>
                        ...
                        <div className="pt-2">
                          <Button
                            ...
                            onClick={() => router.push(`/event/${tour.tourId}`)}
                          >
                            자세히 보기
                          </Button>
                        </div>
                  ...
                  <div className="text-center py-8">
                    <div className="inline-flex items-center space-x-2 text-muted-foreground">
                      ...
                      <div
                        ...
                        style={{ animationDelay: "0.2s" }}
                      ></div>
                      <span className="ml-2 text-sm">더 많은 행사를 불러오는 중...</span>
                    </div>
                  </div>
                )}
                ...
                {!isLoadingEvents && !hasNext && tours.length > 0 && (
                  <div className="text-center py-8">
                    <p className="text-muted-foreground">모든 행사를 확인했습니다</p>
                  </div>
                )}
                ...
                {!isLoadingEvents && hasNext && (
                  <div className="text-center py-4">
                    <p className="text-sm text-muted-foreground">아래로 스크롤하여 더 많은 행사를 확인하세요</p>
                  </div>
                )}
              </CardContent>
            </Card>
      ...
      <footer className="bg-muted/30 py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            ...
            <div className="text-sm text-muted-foreground">© 2025 픽동. All rights reserved.</div>
          </div>
        </div>
      </footer>
    </div>
