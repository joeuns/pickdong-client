...
export function SubscriptionModal({ isOpen, onClose, subscribedRegions, onUnsubscribe, onSubscribe }: SubscriptionModalProps) {
        ...
        try {
          ...
          const updatedAreas = response.areas.map(area => ({
            ...
            subAreas: area.subAreas.map(subArea => ({
              ...subArea,
              subscribe: subscribedRegions[area.name]?.includes(subArea.name) || subArea.subscribe
            }))
          }))
  ...
  }, [isOpen, subscribedRegions])

  const getTotalSubscriptions = () => {
    return areas.reduce((total, area) => 
      total + area.subAreas.filter(subArea => subArea.subscribe).length, 0
    )
  } 

  const handleRegionClick = async (areaCode: string, areaName: string, sigunGuCode: string, subAreaName: string, isSubscribed: boolean) => {
      ...
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto scrollbar-thin scrollbar-thumb-yellow-300 scrollbar-track-gray-100 hover:scrollbar-thumb-yellow-400">
        <DialogHeader>
          <DialogTitle className="flex items-center">
            <MapPin className="w-5 h-5 mr-2 text-primary" />지역 구독 관리
          </DialogTitle>
          <DialogDescription>
            전체 지역을 확인하고 구독할 수 있습니다. 구독한 지역은 노란색으로 표시됩니다.
          </DialogDescription>
        </DialogHeader>
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            {areas.map((area) => {
              const subscribedCount = area.subAreas.filter(subArea => subArea.subscribe).length
              return (
                <div key={area.areaCode} className="space-y-3">
                  <div className="flex items-center justify-between">
                    <h4 className="font-semibold text-lg text-foreground">{area.name}</h4>
                    ...
                    {area.subAreas.map((subArea) => {
                      ...
                      return (
                        <Button
                          ...
                          size="sm"
                          disabled={isUpdating}
                          onClick={() => handleRegionClick(area.areaCode, area.name, subArea.sigunGuCode, subArea.name, subArea.subscribe)}
        ...
        <div className="pt-4 border-t">
          <div className="flex items-center justify-between text-sm text-muted-foreground">
            <span>총 구독 지역</span>
            <span className="font-semibold text-primary">{getTotalSubscriptions()}개</span>
          </div>
        </div>
      </DialogContent>
    </Dialog>
